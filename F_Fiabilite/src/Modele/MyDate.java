...
public class MyDate {
		...
		for(int annee = 2014; annee<2017; annee++) {
			...
			for(int mois = 0; mois<13; mois++) {
				...
				for(int jour = 0; jour<32; jour++) {
					...
					for(int i = 0; i<ligneDate.size(); i++) {
						nbTour ++;
						String date = obtenirJJMMAAAA(ligneDate.get(i));

						int l_annee = obtenirAnnee(date);
						int le_mois = obtenirMois(date);
						int le_jour = obtenirJour(date);

						if(l_annee == annee && le_mois == mois && le_jour == jour) resultat.add(ligneDate.get(i));
					}
				}
			}
		}
	...
	public static ArrayList<String> trierS(ArrayList<String> ligneDate) {
		...
		for(int i = 0; i<60; i++) {
			...
			for(int j = 0; j<ligneDate.size(); j++) {

				String HMS = obtenirHMS(ligneDate.get(j));

				int seconde = obtenirSeconde(HMS);

				if(seconde == i) resultat.add(ligneDate.get(j));
			}
		}
	...
	public static ArrayList<String> trierM(ArrayList<String> ligneDate) {
		...
		for(int i = 0; i<60; i++) {
			...
			for(int j = 0; j<ligneDate.size(); j++) {
				...
				if(minutes == i) resultat.add(ligneDate.get(j));
			}
		}
	...
	public static ArrayList<String> trierH(ArrayList<String> ligneDate) {
		...
		for(int i = 0; i<24; i++) {
			...
			for(int j = 0; j<ligneDate.size(); j++) {
				...
				if(heures == i) resultat.add(ligneDate.get(j));
			}
		}
			...
			for(int m = 0; m<60; m++) {
				...
				for(int s = 0; s<60; s++) {
					...
					for(int i = 0; i<ligneDate.size(); i++) {
						...
						if(heure == h && minute == m && seconde == s) resultat.add(ligneDate.get(i));
					}
				}
			}
		}
