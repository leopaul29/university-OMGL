...
public class TUN_Zone {
	...
	public static void main(String[] args) {
		...
		// Tests de la class zone
		//
		Tests.Design("Controle des zones", 3); {
			...
			Tests.Case("Accesseur de consultation"); {
				...
				Tests.Unit("Dimension [ height = 600.0, width = 300.0 ]", z2.afficherDim());
				Tests.Unit("Dimension [ height = 200.0, width = 800.0 ]", zA.afficherDim());
			...
			Tests.Case("Méthode toString"); {
				...
				Tests.Unit("Zone Text : zone 1 [ Dimension [ height = 500.0, width = 500.0 ], texte d'information ]"
						, z1.toString());
				Tests.Unit("Zone Bloc : paysage [ Dimension [ height = 600.0, width = 300.0 ], chemin d'image ]"
						, z2.toString());
				Tests.Unit("Zone Bloc : diaporama [ Dimension [ height = 200.0, width = 800.0 ], chemin dossierImages ]"
						, z3.toString());
				Tests.Unit("Zone TextAlerte : Alerte rouge [ Dimension [ height = 200.0, width = 800.0 ], URGENT ]"
						, zA.toString());
			}
		}
		...
		Tests.Design("Controle du Stock_zones", 3); {
			...
			Tests.Case("Accesseur de consultation"); {
				...
				//
				Tests.Unit("Dimension [ height = 200.0, width = 800.0 ]", stock_zone.getZone(alerte).afficherDim());
				Tests.Unit("Dimension [ height = 600.0, width = 300.0 ]", stock_zone.getZone(im).afficherDim());
				Tests.Unit("Dimension [ height = 500.0, width = 500.0 ]", stock_zone.getZone(zone1).afficherDim());
				Tests.Unit("Dimension [ height = 200.0, width = 800.0 ]", stock_zone.getZone(diapo).afficherDim());
			...
			Tests.Case("Méthode toString"); {
				...
				// Liste des zones
				//
				String test_toString1 = "Liste de Zones : {paysage=Zone Bloc : paysage [ Dimension [ height = 600.0, width = 300.0 ], chemin d'image ],"
